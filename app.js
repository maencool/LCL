...
const App = {
    ...
    refreshDataFromServer() {
        // Allow refresh on any local network address (localhost, 127.0.0.1, or local IP)
        const isLocal = window.location.hostname === 'localhost' || 
                       window.location.hostname === '127.0.0.1' || 
                       window.location.hostname.startsWith('192.168.');

        if (!isLocal) {
            return; // Skip on GitHub Pages or external sites
        }

        // Add timestamp to bust cache
        const timestamp = new Date().getTime();
        const apiUrl = `${window.location.origin}/api/data?t=${timestamp}`;
        fetch(apiUrl)
            .then(res => res.json())
            .then(data => {
    ...
    // Start auto-refresh interval (localhost only)
    startAutoRefresh() {
        const isLocal = window.location.hostname === 'localhost' || 
                       window.location.hostname === '127.0.0.1' || 
                       window.location.hostname.startsWith('192.168.');
        if (!isLocal) {
            console.log('⏸️ Auto-refresh disabled on GitHub Pages');
            return;
        }
    ...
    handleLogin(e) {
        ...
        // First, refresh data from server to get latest from Supabase
        fetch(`${Storage.API_URL}?t=${new Date().getTime()}`)
            .then(res => res.json())
            .then(data => {
                // Update storage with fresh data
                Storage.cachedData = data;
                localStorage.setItem(Storage.DATA_KEY, JSON.stringify(data));
                console.log('✅ Refreshed data from server before login');
                
                // Now attempt login with fresh data
            ...
            .catch(err => {
                console.warn('⚠️ Could not refresh data from server:', err.message);
                // Proceed with login using cached data
                const result = Auth.login(email, password);
    ...
    updateUI() {
        ...
        if (Auth.isLoggedIn()) {
            ...
            // Get display name - prefer displayName, fallback to email
            const displayName = Auth.currentUser?.displayName && Auth.currentUser.displayName.trim() 
                ? Auth.currentUser.displayName 
                : (Auth.currentUser?.email || 'User');
            
            userDisplay.textContent = `👤 ${displayName}`;
            console.log('✅ UI Updated for user:', displayName, 'Admin:', Auth.isAdmin(), 'Full user:', Auth.currentUser);

            // Show admin link if user is admin
            if (Auth.isAdmin()) {
                document.querySelector('.admin-link').classList.remove('hidden');
                console.log('✅ Admin recognized - showing admin panel');
            } else {
    ...
    loadSettings() {
        ...
        const settings = Storage.getSettings() || { theme: 'dark', darkBackground: true };
        ...
        if (themeSelect) {
            ...
            console.log('✅ Theme select loaded:', themeSelect.value);
        ...
        if (backgroundToggle && settings.darkBackground !== undefined) backgroundToggle.checked = settings.darkBackground;
